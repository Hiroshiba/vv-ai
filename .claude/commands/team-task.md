...
3. reviewer からレビュー結果ファイルのパスだけを受け取る。内容は一切受け取らない
4. ファイルパスをユーザーに提示する

## レビュー結果の処理

1. reviewer を shutdown する
2. implementer にレビュー結果ファイルのパスを伝え、「review-triage スキルを実行してください。修正するかどうかはあなたが判断してください」と依頼する
3. implementer のプラン承認リクエストが来たら承認する
4. implementer 完了後、新しい reviewer-(N+1) を spawn して「review-diff スキルを実行してください」と伝える
5. ファイルパスをユーザーに提示する
6. ユーザーが OK したら完了フェーズへ進む。問題を指摘したら 2 に戻る
