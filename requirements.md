...
AI がコメント、要件定義、設計、レビューなどのユーザー向け文章を書くときは、コード内の英字語や識別子を必要なくそのまま使わない。一般的な英単語は、ドメイン用語化せず自然な日本語表現へ寄せる。表の用語欄には、コード内表現ではなくユーザーに見せたい表現を書く。コードシンボル、ファイルパス、コマンド、設定値、API 名など実装詳細を指す場合は、英字語を使ってよい。
